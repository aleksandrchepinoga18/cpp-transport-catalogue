...
Node LoadArray(std::istream& input) {
    ...
    for (char symbol; input >> symbol && symbol != ']';) {
        ...
        if (input >> symbol) {
            // Перерыв, если это конец массива
            if (symbol == ']')
                break;
            //Если разбираемый элемент не был последним, но после него нет ","
            if (symbol != ',')
...
Node LoadString(std::istream& input) {
    ...
    while (true) {
        ...
        // Если "не является частью" тока - это конец строки
        if (current == '"') {
            ++position;
            break;
        } else if (current == '\\') {
            // Если строка начинается с '\\' -> это escape-символ \n \t или \r
            if (++position == end)
